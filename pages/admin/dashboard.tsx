...
import { 
  ...
  FaChevronLeft, FaChevronRight, FaExclamationTriangle, FaUsers, FaUserCircle
} from 'react-icons/fa';
...
type TabType = 'overview' | 'products' | 'contacts' | 'users';
...
const AdminDashboard = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [productsData, statsData, messagesData, contactStatsData, usersData] = await Promise.all([
      ...
      setDashboardStats({ ...statsData, users: usersData.length });
  ...
  const handleSearchUsers = (query: string) => {
    ...
    const filtered = users.filter(user => 
      ...
      (user.company_name && user.company_name.toLowerCase().includes(lowercaseQuery))
    );
  ...
  const menuItems = [
    ...
    { id: 'users' as TabType, name: 'Manajemen User', icon: FaUsers },
  ];
  ...
  return (
    <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6 laptop:gap-3 mb-8 laptop:mb-4">
                ...
                >
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      <p className="text-sm text-gray-600 dark:text-gray-300">Total User</p>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-4">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-3 border dark:border-gray-600 rounded-lg focus:outline-none focus:border-seltronik-red dark:bg-gray-700 dark:text-white"
                  />
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-gray-800 rounded-lg shadow overflow-hidden">
                      ...
                      {filteredUsers.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="px-4 lg:px-6 py-8 text-center text-gray-500 dark:text-gray-400">
                            {userSearchQuery ? 'Tidak ada user yang sesuai dengan pencarian' : 'Belum ada user terdaftar'}
                          </td>
                        </tr>
                      ) : (
                        filteredUsers
                          ...
                          .map((user) => (
                            ...
                            >
                              <td className="px-4 lg:px-6 py-4 whitespace-nowrap">
                                <div className="flex items-center">
                                  <div className="flex-shrink-0 h-10 w-10 bg-seltronik-red/10 rounded-full flex items-center justify-center">
                                    <FaUserCircle className="h-6 w-6 text-seltronik-red" />
                                  </div>
                                  <div className="ml-4">
                              ...
                              <td className="px-4 lg:px-6 py-4 whitespace-nowrap">
                                <div className="text-sm text-gray-900 dark:text-white">{user.company_name || '-'}</div>
                              </td>
                              <td className="px-4 lg:px-6 py-4 whitespace-nowrap">
                ...
                {filteredUsers.length > usersPerPage && (
                  <div className="px-4 lg:px-6 py-4 border-t dark:border-gray-700 flex items-center justify-between">
                    <div className="text-sm text-gray-600 dark:text-gray-400">
                      Menampilkan {((currentUserPage - 1) * usersPerPage) + 1} - {Math.min(currentUserPage * usersPerPage, filteredUsers.length)} dari {filteredUsers.length} user
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-gray-600 dark:text-gray-400">
                        Halaman {currentUserPage} dari {Math.ceil(filteredUsers.length / usersPerPage)}
                      </span>
                      <button
                        onClick={() => setCurrentUserPage(prev => Math.min(Math.ceil(filteredUsers.length / usersPerPage), prev + 1))}
      ...
      <Modal
        ...
        title="Detail User"
        subtitle="Informasi lengkap customer terdaftar"
        ...
        {selectedUser && (
          <div className="space-y-6">
            ...
            <div className="flex items-start gap-4 pb-4 border-b dark:border-gray-700">
              <div className="flex-shrink-0 h-16 w-16 bg-seltronik-red/10 rounded-full flex items-center justify-center">
                <FaUserCircle className="h-10 w-10 text-seltronik-red" />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Perusahaan</span>
                <p className="text-gray-900 dark:text-white font-medium mt-1">{selectedUser.company_name || '-'}</p>
              </div>
              <div>
                <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Telepon</span>
                <p className="text-gray-900 dark:text-white font-medium mt-1">{selectedUser.phone || '-'}</p>
              </div>
              <div>
                <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Tanggal Daftar</span>
            ...
            {/* Purpose/Interest */}
            <div>
              <span className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Tujuan / Minat</span>
              <div className="mt-2 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg">
                <p className="text-gray-700 dark:text-gray-300 whitespace-pre-wrap">
                  {selectedUser.purpose_interest || 'Tidak ada informasi'}
                </p>
              </div>
            </div>
